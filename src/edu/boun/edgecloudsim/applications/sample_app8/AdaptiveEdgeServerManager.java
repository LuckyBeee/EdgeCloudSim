...
public class AdaptiveEdgeServerManager extends EdgeServerManager{
	private int hostIdCounter;

	public AdaptiveEdgeServerManager() {
		hostIdCounter = 0;
	}
	...
	public double getAvgUtilization(){
		...
		for(int i= 0; i<localDatacenters.size(); i++) {
			...
			for (int j=0; j < list.size(); j++) {
				...
				List<EdgeVM> vmArray = AdaptiveSimManager.getInstance().getEdgeServerManager().getVmList(host.getId());
				//for each vm...
