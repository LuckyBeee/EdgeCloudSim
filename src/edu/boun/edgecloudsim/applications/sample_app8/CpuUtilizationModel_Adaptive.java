...
public class CpuUtilizationModel_Adaptive implements UtilizationModel {
	private AdaptiveTask task;
	
	public CpuUtilizationModel_Adaptive(){
	}
	...
	public void setTask(AdaptiveTask _task){
		task=_task;
	}
	...
	public double predictUtilization(SimSettings.VM_TYPES _vmType){
		...
		if(_vmType == SimSettings.VM_TYPES.EDGE_VM)
			...
			AdaptiveSimLogger.printLine("Unknown VM Type! Terminating simulation...");
			System.exit(1);
		}
		return SimSettings.getInstance().getTaskLookUpTable()[task.getTaskType()][index];
	}
